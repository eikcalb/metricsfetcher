...
public:
    ...
    void ExecuteJavaScript(const std::string& javascriptCode) {
        if (!ctx) {
            throw std::runtime_error("Duktape context is not initialized.");
        }

        // Execute the JavaScript code
        if (duk_peval_string(ctx, javascriptCode.c_str()) != 0) {
    ...
    void SetupCounter(std::string counterName) {
        PdhAddEnglishCounter(queryHandle, LPCWSTR(counterName.c_str()), 0, &counter);
    }
